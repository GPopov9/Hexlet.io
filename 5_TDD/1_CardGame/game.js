...
const run = (player1, player2, cards) => {
  const iter = (health1, name1, health2, name2, order, log) => {
    ...
    const cardRandom = random(cards); 
    const cardName = car(cardRandom);
    const cardPower = cdr(cardRandom)();

    const newOrder = order === 1 ? 2 : 1; 

    if (order === 1) {
      const message = `Игрок ${name1} применил ${cardName} против ${name2} и нанес урон ${cardPower}`;
      const newHealth = health2 - cardPower;
      const newLog = consList(cons(cons(health1, newHealth), message), log);
      return iter(health1, name1, newHealth, name2, newOrder, newLog); 
    }

    const message = `Игрок ${name2} применил ${cardName} против ${name1} и нанес урон ${cardPower}`;
    const newHealth = health1 - cardPower;
